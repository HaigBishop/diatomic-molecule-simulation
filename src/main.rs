...
fn main() {
    ...
    let r0_si_harm: f32 = ((2.0 * KB * TEMP_K) / H2_K_SI).sqrt();
    let r0_si_morse: f32 = (1.0 - (H2_K_SI * r0_si_harm * r0_si_harm / (2.0 * H2_D_SI)).sqrt()).ln() / (-H2_ALPHA_SI);
    ...
    let mut state = init_harmonic_osc(r0_a0_harm, H2_K_AU, H2_M_AU);
    ...
    for sim_iter in 0..N_STEPS {
        ...
        calculate_harmonic_osc_state(&mut state, r0_a0_harm, H2_K_AU, H2_M_AU);
    ...
    // Initialize the simulation system
    let mut state = init_morse_potential(r0_a0_morse, H2_D_AU, H2_ALPHA_AU, H2_M_AU);
    // Create a vector to store the history of states
